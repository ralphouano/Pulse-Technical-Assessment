...
import { Paperclip, FileText, Download, Send, Video, PhoneOff, Upload, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function ChatPanel({
  ...
  }, [activeImageId, activeIndex, viewableImages, handleNext, handlePrev, handleClose]);
                ...
                {isImageFile && (
                  <div className="flex flex-col gap-2">
                    <img
                      ...
                      onClick={() => setActiveImageId(m.id)}
                    />
                    <a
                      href={m.downloadUrl}
                      download={m.text.replace("File ready: ", "").replace("File sent: ", "").replace("📁 ", "")}
                      className={`text-xs hover:underline flex items-center gap-1 mt-1 justify-end font-bold ${m.mine ? "text-emerald-950" : "text-emerald-400"}`}
                    >
